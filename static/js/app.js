...
byId("filter-form").addEventListener("submit", async (event) => { event.preventDefault(); const params = nonEmptyFormParams(event.currentTarget); try { const result = await request(`/api/papers?${params}`); renderPapers(result.papers); } catch (error) { byId("papers-summary").textContent = error.message; } });
